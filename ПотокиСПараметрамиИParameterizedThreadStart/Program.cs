...
Сначала определяем специальный класс Person, объект которого будет передаваться во 
...
используемые методы и переменные в специальном классе, а в основной программе 
запускать поток через ThreadStart. Например:

*/


Person2 tom2 = new Person2("Tom", 37);
// создаем новый поток
Thread myThread5 = new Thread(tom2.Print);
myThread.Start();
