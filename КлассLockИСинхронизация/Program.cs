...
Метод TryEnter() также осуществляет вход в критическую секцию. Этот метод возвращает 
булевое значение - true или false. Если метод возвращает true, текущий поток является 
единственным потоком, который удерживает блокировку. Если метод возвращает false, 
то поток не ждет освобождения блокировки. Например:

*/

static void tryEnterEx() 
{
    int x = 0;  // некоторый общий ресурс
    Lock _lockObj = new(); // объект-заглушка для синхронизации доступа
    // запускаем пять потоков
...
Мы видим, что действия критической секции были выполнены только одним потоком. 
Потому что у нас потоки запускаются одновременно. Первый поток получил блокировку 
(метод _lockObj.TryEnter() возвратил true) и этот поток стал выполнять действия 
из критической секции. Поскольку блокировка стала занята, для других потоков 
метод _lockObj.TryEnter() возвратил false, и они не стали выполнять действия из 
критической секции и не стали ждать освобождения блокировки.
...
Еще один способ демонстрирует метод EnterScope(), который является рекомендуемым 
способом (с точки зрения Microsoft). Он также осуществляет вход в критическую секцию. 
Если один поток уже захватил блокировку и вошел в эту секцию, другие потоки ждут 
освобождения блокировки. Данный метод прежде всего предназначен для использования в 
конструкции using, которая автоматически освобождает все связанные ресурсы:

*/

static void EnterScopeEx()
...
Причем в данном случае не надо вызывать метод Exit(), выход из критической секции и освобождение блокировки
происходит автоматически. Консольный вывод:
